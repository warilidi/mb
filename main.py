...
def main() -> None:
    ...
    app.add_handler(CallbackQueryHandler(handlers.activity_callback_handler, pattern="^(actselect_|actq_|actcustom_|act_back|cancel_action)"))
    app.add_handler(CallbackQueryHandler(handlers.leaderboard_callback_handler, pattern="^top_"))

    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handlers.text_input_handler))
