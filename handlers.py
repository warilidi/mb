...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"Бот помогает превратить ежедневные полезные привычки в систему прокачки персонажа.\n\n"
        f"**Правила начисления:**\n"
        f"• Тренировка: **+5 к Силе**\n"
        f"• Чтение книги: **+10 к Интеллекту**\n"
        f"• Пробежка: **+5 к Ловкости**\n"
        f"• Медитация: **+5 к Мудрости**\n"
        f"• Изучение языка: **+8 к Интеллекту**\n"
        f"• Питьевой режим: **+3 к Здоровью**\n"
        f"• Полноценный сон: **+5 к Здоровью**\n\n"
        f"Записывай активность, поднимай уровень и конкурируй в таблице лидеров."
    )
...
async def activity_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("act_"):
        act_key = data.replace("act_", "")
        res = db.add_activity(user.id, act_key)
